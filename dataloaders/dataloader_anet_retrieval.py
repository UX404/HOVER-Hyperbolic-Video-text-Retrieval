...
class Anet_Test_DataLoader(Dataset):
    ...
    def _load_metadata(self):
        ...
        with open(os.path.join(self.data_dir, 'test_tree_shallow.json'),'r') as f:
            shallow_annos = json.load(f)
        for vid, video_anno in annotations.items():
            video_anno_shallow = shallow_annos[vid.split('#')[0]]
            if 'leaf_num' in video_anno:
                ...
                if child_num != 0 and child_num != CHILDLIMIT and len(video_anno_shallow['sentences']) != child_num + 1:
                    continue
                if child_num == CHILDLIMIT and len(video_anno_shallow['sentences']) <= CHILDLIMIT:
                    continue
                node_cnt.append(len(video_anno_shallow['sentences'])-1)
