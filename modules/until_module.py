...
class BinaryTreeLoss(nn.Module):
    ...
    def forward(self, embeded_tree):
        parent = embeded_tree[0]
        child = embeded_tree[1]
...
class NormLoss(nn.Module):
    def __init__(self, c=1, alpha=1, epsilon=0.1):
    ...
    def forward(self, parent, child):
        ...
        score = - self.alpha * F.relu(child_norm - parent_norm + self.epsilon)
        loss = score.mean()
        return loss

class TemporalLoss(nn.Module):
    def __init__(self, c=1, alpha=0.2, epsilon=0.1, dim=512):
        super(TemporalLoss, self).__init__()
        self.c = c
        self.alpha = alpha
        self.epsilon = epsilon
        self.base = nn.Parameter(torch.ones(dim), requires_grad=True)

    def forward(self, left_child, right_child):
        left_child_cos = (left_child * self.base).sum(dim=1) / left_child.norm(dim=1)
        right_child_cos = (right_child * self.base).sum(dim=1) / right_child.norm(dim=1)
        score = - self.alpha * F.relu(left_child_cos - right_child_cos + self.epsilon)
        loss = score.mean()
        return loss
