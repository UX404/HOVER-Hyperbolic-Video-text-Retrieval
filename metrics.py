...
def compute_metrics(x, order=None, above=False):
    sims = deepcopy(x)
    ...
    ind = np.where(ind == 0)
    ind = ind[1]
    # ind = np.array([i.index(0) for i in ind.tolist()])
    if order != None:
      # print(len(order[1]), len(ind))
      # print(order, ind)
      target_order, all_order = deepcopy(order)
      if above:
        ind = ind[np.array(all_order)>=target_order]
      else:
        ind = ind[np.array(all_order)==target_order]
      # print(target_order, '#', len(ind))
    if len(ind) == 0:
      return None
    ...
    metrics['MRR'] = np.sum(1 / (ind + 1)) / len(ind)
...
def print_computed_metrics(metrics):
    ...
    mrr = metrics['MRR']
    mr = metrics['MR']
    print('R@1: {:.4f} - R@5: {:.4f} - R@10: {:.4f} - MRR: {} - Median R: {}'.format(r1, r5, r10, mrr, mr))
...
def tensor_text_to_video_metrics(sim_tensor, top_k = [1,5,10]):
    ...
    results['MRR'] = float(torch.mean(1 / (valid_ranks + 1)))
